...
def parse_amfr_fixtures_page(url, headers):
	...
	for tr in trs:
		...
		fixture['home_team_name'] = fixtureInfo.find('div', class_='league-item__team-inn is-right').text.strip()
		fixture['away_team_name'] = fixtureInfo.find('div', class_='league-item__team-inn is-left').text.strip()
		try:
			score = fixtureInfo.find('div', class_='league-item__score').text.strip()
			fixture['home_team_goals'] = int(score.split('-')[0].strip())
			fixture['away_team_goals'] = int(score.split('-')[1].strip())
		except Exception as e:
			pass
		
		fixture['id'] = int(tr['data-link'].replace('/match/', '').replace('/', ''))
...
def parse_amfr_fixtures():
	...
	writeJson(fixtures, 'superliga/fixtures.json')
